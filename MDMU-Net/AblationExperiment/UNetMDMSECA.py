...
class MDCA(nn.Module):
    ...
    def get_channels(self,x):
        b, c, h, w, d = x.size()
        d = self.channel_d(x)
        fc_d = self.fc_d(self.ave(d).view(b,c)) + self.fc_d(self.max(d).view(b,c))

        h = self.channel_h(x)
        fc_h = self.fc_h(self.ave(h).view(b,c)) + self.fc_h(self.max(h).view(b,c))

        w = self.channel_w(x)
        fc_w = self.fc_w(self.ave(w).view(b,c)) + self.fc_w(self.max(w).view(b,c))

        fc_dhw = fc_d + fc_h + fc_w
    ...
    def forward(self,x):
        source_x = x
        attention = self.get_channels(x) * source_x
        return attention
...
class MDMSE(nn.Module):
    ...
    def forward(self,x):
        ...
        Attention_result = self.drop(Attention_result)
        result = Attention_result + x1
...
class UNetMDMSECA(nn.Module):
    def __init__(self,class_num=3):
        super(UNetMDMSECA,self).__init__()
